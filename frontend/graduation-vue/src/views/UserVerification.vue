<template>
  <div class="verification-container">
    <n-card class="verification-card" title="个人实名认证">
      <n-form
        ref="formRef"
        :model="formData"
        :rules="rules"
      ...
      >
        <n-form-item label="认证类型" path="verification_type">
          <n-radio-group v-model:value="formData.verification_type" name="verification_type">
            <n-radio value="INDIVIDUAL">个人认证</n-radio>
          </n-radio-group>
        </n-form-item>
        ...
        <n-form-item label="学信网截图" path="student_card" required>
          <n-upload
            v-model:file-list="studentCardList"
            :max="1"
            accept=".jpg,.jpeg,.png"
...
import { useMessage, NCard, NForm, NFormItem, NInput, NButton, NSpace, NUpload, NDescriptions, NDescriptionsItem, NTag, NTooltip, NIcon } from 'naive-ui'
...
const handleStudentCardChange = (options) => {
  if (options.fileList && options.fileList.length > 0) {
    formData.value.student_card = options.fileList[0].url
  }
}
...
const handleSubmit = async () => {
  try {
    ...
    if (!formData.value.student_card) {
      message.error('请上传学信网截图')
      return
    }
  ...
  try {
    ...
    if (response.data) {
      ...
      if (response.data.status === 'APPROVED') {
        ...
        userInfo.verification_type = 'INDIVIDUAL'
        localStorage.setItem('userInfo', JSON.stringify(userInfo))
        message.success('实名认证已通过，现在可以发布文章和投递简历了')
      }
    }
  } catch (error) {
