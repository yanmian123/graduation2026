...
def create_notification(recipient, notification_type, title, message, related_object_id=None, related_object_type=None, comment_id=None):
    ...
    Returns:
        创建的Notification对象
    """
    print(f"创建通知: 接收者={recipient.username}, 类型={notification_type}, 标题={title}")
    ...
    print(f"通知已创建: ID={notification.id}, 接收者ID={notification.recipient.id}")
...
def send_notification_via_websocket(notification):
    ...
    """
    print(f"开始发送WebSocket通知: 通知ID={notification.id}, 接收者ID={notification.recipient.id}")
    
    # 序列化通知数据
    serializer = NotificationSerializer(notification)
    notification_data = serializer.data
    print(f"通知数据序列化完成: {notification_data}")
    
    # 获取Channel Layer
    channel_layer = get_channel_layer()
    print(f"获取Channel Layer: {channel_layer}")
    
    # 构建用户通知组名称
    group_name = f'user_notifications_{notification.recipient.id}'
    print(f"目标通知组: {group_name}")
    ...
    try:
        ...
        print(f"WebSocket通知已发送到组: {group_name}")
